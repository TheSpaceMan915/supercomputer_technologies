...
#define TEST_ASSERT_DOUBLE_WITHIN(eps, expected, actual) do {   double diff = ((actual) > (expected)) ? ((actual)-(expected)) : ((expected)-(actual));   if (!(diff <= (eps))) {     printf("Assertion failed: |%s - %s| <= %s (%s:%d)\n", #actual, #expected, #eps, __FILE__, __LINE__);     ++UnityFailCount;   }   ++UnityTestCount; } while(0)
